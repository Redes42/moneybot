...
def get_help(data: StageData) -> str:
    return (
        'Этот бот помогает быстро посчитать, кто кому и сколько должен после вечеринки.\n\n'
        '👉 Обычно участников много, и расходы у всех разные. Кроме того, можно учитывать особые случаи: например, если кто-то почти не ел, ему можно задать коэффициент (к-т) 0.5. Если человек пришёл с семьёй, удобно добавить всю семью одной записью и указать общий к-т.\n\n'
        '<b>Как пользоваться:</b>\n'
        '1. Заполните базу участников: укажите имя и к-т (обычно 1.0). Это своего рода адресная книга для будущих встреч. Семьи можно добавлять одной записью, например «Ивановы» с к-том 2.0.\n'
        '2. Создайте вечеринку и добавьте только тех, кто пришёл. При необходимости измените их к-т.\n'
        '3. Запустите расчёт. Бот покажет распределение расходов и главное — кто, кому и сколько должен перевести.\n\n'
        
        '<b>Особенность:</b>\n'
        'К-ты округляются на 0.5 (только при визуализации количества участников в отчёте, но не при расчёте). Например, участник с к-том 1.5 отображается в отчёте как 2 человека, а 1.49999 — как 1. Это компромисс, который подходит для большинства случаев.'
